...
router.get("/user/:id", async (req, res) => {
  try {
    const response = await userModel.find({ id: req.params.id });
    if (response) {
...
router.put("/user/:id", async (req, res) => {
  try {
    const response = await userModel.findOneandUpdate_(
      { id: req.params.id },req.body,{ new: false });
    if (!response) {
      return res.status(404).json({ message: "User not found", data: response });
    }
    res.status(200).json({ message: "User updated successfully", data: response });
...
router.delete("/user/:id", async (req, res) => {
  try {
    const response = await userModel.deleteOne({ id: req.params.id });
